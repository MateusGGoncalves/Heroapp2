...
export default class PersonList extends React.Component {
  ...
  render() {
    ...
    return  (
      ...
      {arr1.map(hero2 => <img width="85%" border="3px" src={hero2.url} alt="imagem"/>)}
      {arr2.map(hero3 => <div className="Status">Inteligência:{hero3.intelligence}
      <p>Força:{hero3.strength}</p>
      <p>Velocidade:{hero3.speed}</p>
      <p>Defesa:{hero3.durability}</p>
      <p>Poder:{hero3.power}</p>
      <p>Combate:{hero3.combat}</p></div>)}
      </div>
    )
  }
}